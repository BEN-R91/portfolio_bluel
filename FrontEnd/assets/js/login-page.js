...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelector("form")?.addEventListener("submit", async (event) => {
    ...
    const response = await fetch(url_api, {
      ...
      body: JSON.stringify({ email, password }) // on transforme les données en JSON (chaine de caractères pour que le serv comprenne jemploie JSON.stringify) et donc données concernées email et pwd. body= corp/contenu de la requete
    });
